...
var QRS = function QRS ( options ) {
	...
	this.query = function ( method, endpoint, urlParams, body ) {
		...
		if ( !validConfig ) {
			...
			if ( that.config.authentication === 'certificates' ) {
				...
				if (that.config.passphrase) {requestOptions.passphrase = that.config.passphrase;}
			}
	...
	// Extensions
	// ****************************************************************************************
	//Todo: should be implemented in a generic way loading all sugar extensions, like in sense-go
	var m = require( './sugar/ep-mime' );
	this.mime = new m( this );
