...
function Mime ( base ) {
	...
	var get = function ( filter ) {
		...
		var defer = Q.defer();
		var url = 'mimetype/full';
		var queryParams = [];
		if ( !_.isEmpty( filter ) ) {
			queryParams.push( {"key": "filter", "value": filter} );
		}
		that.baseClass.get( 'mimetype/full', queryParams )
			.then( function ( data ) {
	...
	var addMultiple = function ( mimeTypeDefs ) {
		...
		// http://joost.vunderink.net/blog/2014/12/15/processing-an-array-of-promises-sequentially-in-node-js/
		if ( mimeTypeDefs && _.isArray( mimeTypeDefs ) ) {

			//var result = Q();
			//mimeTypeDefs.forEach( function ( mimeTypeDef ) {
			//	result = result.then( add.bind( null, mimeTypeDef ) );
			//} );
			//return result;
	...
	var getUpdateOrInsert = function ( mimeTypeDef, listMimeTypes ) {
		...
		if ( tmp.length === 1 ) {
			...
			//updatedDef.extensions = _.unique(updatedDef.extensions);
